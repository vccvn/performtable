jQuery(document).ready(function($) {
	...
	(function(){
		...
		epkb.find( '#epkb_show_sequence' ).on( 'click', function() {
			...
			epkb_send_ajax( postData, function( response ) {console.log(response);
				$( '.eckb-top-notice-message' ).remove();
				if ( typeof response.message !== 'undefined' ) {
					clear_bottom_notifications();
					$( 'body' ).append( response.message );
	...
	// About KB Settings 'Click here' button open Full Editor Tab
	$( document ).on( 'click', '.epkb-admin__form .epkb-admin__form-tab-wrap--about-kb .epkb-admin__form-tab-content-desc__link', function( e ) {
		e.preventDefault();
	 ...
	 */
	$('.epkb-admin__kb__form-save__button').on( 'click', function(){

		let $wrap = $(this).closest('.epkb-admin__form');
	...
	$( document ).on( 'click', '#toc_locations input', function() {
		...
		let target_value = $( this ).prop( 'checked' ) ? '1' : '0';
		$( target_option_container ).find( 'input[value="' + target_value + '"]' ).closest( '.epkb-label' ).trigger( 'click' );
	} );
	$( document ).on( 'click', '#toc_left .epkb-input-container, #toc_right .epkb-input-container', function() {
		let current_input = $( this ).find( 'input:checked' );
		if ( ! current_input.length ) {
			return;
		}
		let target_input = $( '#toc_locations_group input[value="' + $( current_input ).prop( 'name' ) + '"]' );
		if ( ! $( target_input ).prop( 'checked' ) ) {
			$( target_input ).closest( '.epkb-label' ).trigger( 'click' );
		}
	} );
