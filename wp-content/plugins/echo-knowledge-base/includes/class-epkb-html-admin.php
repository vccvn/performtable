	...
	private static function admin_single_primary_tab_content( $page_view ) {
		...
		if ( ! empty( $page_view['vertical_tabs'] ) && is_array( $page_view['vertical_tabs'] ) ) {      ?>
			...
			<div class="epkb-admin__form">
				<div class="epkb-admin__form__save_button"> <?php

					// TODO remove notification/ Also remove css from _admin_form
					if ( ! EPKB_Utilities::is_new_user('9.20.0') ) {
						EPKB_HTML_Forms::notification_box_middle( array(
							'type'  => 'info',
							'title' => 'Update to KB Configuration Screens', 'echo-knowledge-base',
							'desc'  =>  'We are introducing basic settings to allow users to quickly configure a subset of KB features and colors, accessible on the left sidebar on this page. ' .
							            'The full Visual Editor is still available under the tab called "Full Editor."'
						) );
					}   ?>
