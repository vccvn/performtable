...
class EPKB_KB_Config_Category {
	...
	public function save_category_fields( $term_id, $tt_id ) {
		...
		$categories_icons = self::get_category_icons_option( $this->kb_id );
		$old_term_icon = empty( $categories_icons[$term_id] ) ? [] : $categories_icons[$term_id];
		...
		// mark category
		if ( array_diff( $categories_icons[$term_id], $old_term_icon ) ) {
			update_term_meta( $term_id, EPKB_Icons::CATEGORY_ICON_USER_CHANGED_FLAG, 1 );
		}

		EPKB_Utilities::save_kb_option( $this->kb_id, EPKB_Icons::CATEGORIES_ICONS, $categories_icons );
	}
	...
	public static function remove_missing_terms_and_images_from_categories_icons( $kb_id ) {
		...
		EPKB_Utilities::save_kb_option( $kb_id, EPKB_Icons::CATEGORIES_ICONS, $categories_icons_filtered );
	}
}