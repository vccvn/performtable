...
class EPKB_Editor_Utilities {
	...
	public static function get_editor_urls( $kb_config, $main_page_zone_name='', $article_page_zone_name='', $archive_page_zone_name='', $use_backend_mode = true, $preopen_setting = '' ) {
		...
		if ( EPKB_Utilities::is_advanced_search_enabled() ) {
			...
			if ( ! empty( $posts ) && ! empty( $first_main_page_url ) ) {
				...
				$search_url = empty( $first_main_page_url )
					? ''
					: add_query_arg( array( 'preopen_zone' => 'search_zone', 'action' => 'epkb_load_editor', _x( 'kb-search', 'search query parameter in URL', 'echo-advanced-search' ) => $search_query ), $first_main_page_url );
			}
		}
	...
	public static function epkb_front_end_editor_type() {
		...
		if ( $editor_type == 'main-page' && EPKB_Utilities::is_advanced_search_enabled() && EPKB_Utilities::get( _x( 'kb-search', 'search query parameter in URL', 'echo-advanced-search' ) ) ) {
