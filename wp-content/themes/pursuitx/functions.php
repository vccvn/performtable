 ...
 */
include( get_template_directory() . '/lib/pursuit-kirki.php');
/**
...
add_filter('pursuit_theme_setup_wizard_username', 'pursuit_set_theme_setup_wizard_username', 10);
add_filter('pursuitchildtheme_theme_setup_wizard_username', 'pursuit_set_theme_setup_wizard_username', 10);
if( ! function_exists('pursuit_set_theme_setup_wizard_username') ){
    function pursuit_set_theme_setup_wizard_username($username){
        return 'themovation';
    }
}
...
add_filter('pursuit_theme_setup_wizard_oauth_script', 'pursuit_set_theme_setup_wizard_oauth_script', 10);
add_filter('pursuitchildtheme_theme_setup_wizard_oauth_script', 'pursuit_set_theme_setup_wizard_oauth_script', 10);
if( ! function_exists('pursuit_set_theme_setup_wizard_oauth_script') ){
    function pursuit_set_theme_setup_wizard_oauth_script($oauth_url){
...
add_filter('pursuit_theme_setup_wizard_default_content', 'pursuit_theme_setup_wizard_default_content_script', 10);
add_filter('pursuitchildtheme_theme_setup_wizard_default_content', 'pursuit_theme_setup_wizard_default_content_script', 10);
if( ! function_exists('pursuit_theme_setup_wizard_default_content_script') ){
    function pursuit_theme_setup_wizard_default_content_script($default){
...
// Envato WP Theme Setup Wizard
//add_filter( 'pursuit_enable_setup_wizard', '__return_true' );
//add_filter( 'pursuitchildtheme_enable_setup_wizard', '__return_true' );


/*
 * Pre install check.
 * 1. Make sure we are not upgrading from Pursuit Classic or at least warn of potential issues. Provide override.
...
if ( ! function_exists( 'th_install_safety_check' ) ) :
    function th_install_safety_check() {
        ...
        // Check if we may be upgrading from Pursuit Classic, exit and warn, provide helpful instructions.
        ...
        foreach ($th_themes_installed as $th_theme) {
            ...
            if($th_theme->get( 'Name' ) > ""){
                ...
                if($th_theme_name === 'pursuit' && $th_theme->get( 'Version') < 2 && $th_theme->stylesheet > "" && TH_PREVENT_PURSUIT_UPGRADE){

                    add_action( 'admin_notices', 'th_admin_notice_noupgrade' );
                    function th_admin_notice_noupgrade() {
                        ?>
                        <div class="update-nag">
                            <?php _e( 'Hello, we ran into a small problem, it looks like you are trying to upgrade from an earlier version of Strauts, Version 2. You can still upgrade but please be advised that these two versions are not developed under the same framework and so your existing content will not be migrated.', 'pursuit'); ?> <?php _e( 'If you need help, please contact the <a href="https://themovation.ticksy.com/" target="_blank">Pursuit support team here.</a> or <a href="https://themovation.ticksy.com/article/12056/" target="_blank">read the guide on updating Pursuit V2.</a>', 'pursuit' ); ?> <br />
                        </div>
                        <?php
                    }
...
function check_theme_setup($old_theme_name, $old_theme = false){
    ...
    if ( version_compare(PHP_VERSION, TH_REQUIRED_PHP_VERSION, '<') ) :
        ...
        function th_admin_notice_phpversion() {
            ?>
            <div class="update-nag">
                <?php _e( 'Hello, we ran into a small problem, but it\'s an easy fix. Your version of <strong>PHP</strong>', 'pursuit'); ?> <strong><?php echo PHP_VERSION; ?></strong> <?php _e( 'is unsupported. We recommend <strong>PHP 7+</strong>, however, the theme should work with <strong>PHP</strong>','pursuit') ?> <strong><?php echo TH_REQUIRED_PHP_VERSION; ?>+</strong>. <?php _e( 'Please ask your web host to upgrade your version of PHP before activating this theme. If you need help, please contact the <a href="https://themovation.ticksy.com/" target="_blank">Pursuit support team here.</a>', 'pursuit' ); ?> <br />
            </div>
            <?php
        }
