...
if ( ! function_exists( 'th_bittersweet_pagination' ) ) {
	function th_bittersweet_pagination()
	{
		...
		$pages = paginate_links(array(
			...
			'total' => $total,
			'type' => 'array',
			'prev_text' => esc_html__('Newer posts &rarr;', 'pursuit'),
			'next_text' => esc_html__('&larr; Older posts', 'pursuit'),
		));
    ...
    if ($th_screen->id === 'toplevel_page_masterslider') {
        ...
        if ( get_user_meta( $user_id, 'th_ms_install_dismissed', true ) !== '1' ) { ?>
            <div class="notice notice-info">
                <p><?php _e('Looking for the sliders from the live demo? <a href="http://themovation.helpscoutdocs.com/article/198-master-slider-activation" target="_blank">Check out this article for how to import them</a>.', 'pursuit'); ?> <a class="th-dismiss" href="?page=masterslider&th-ms-install-dismissed">Dismiss</a></p>
            </div>
            <?php
...
function themo_child_theme_setup() {
    load_child_theme_textdomain( 'pursuit', get_stylesheet_directory() . '/languages' );
}
...
function themo_custom_excerpt_more( $output ) {
	if ( (has_excerpt() || themo_has_more()) && ! is_attachment() && get_post_type() != 'themo_tour' && get_post_type() != 'themo_portfolio') {
		$output .= ' &hellip; <a href="' . esc_url(get_permalink()) . '">' . esc_html__('Read More', 'pursuit') . '</a>';
	}
	return $output;
}
...
function themo_read_more_link() {
	if (get_post_type() != 'themo_tour' && get_post_type() != 'themo_portfolio') {
		return ' &hellip; <a href="' . esc_url(get_permalink()) . '">' . esc_html__('Read More', 'pursuit') . '</a>';
	}
...
function themo_ot_meta_box_post_format_quote($array,$pages) {
	...
	$array['fields'] = array(
		...
		array(
			...
			'label'   => '',
			'desc'    => esc_html__( 'Source Name (ex. author, singer, actor)', 'pursuit' ),
			'std'     => '',
			'type'    => 'text'
		),
		array(
			...
			'label'   => '',
			'desc'    => esc_html__( 'Source Title (ex. book, song, movie)', 'pursuit' ),
			'std'     => '',
			'type'    => 'text'
		));
	return $array;
...
function themo_ot_meta_box_post_format_audio($array,$pages) {
	...
	$array['fields'] = array(
		array(
			...
			'desc'    => esc_html__( 'Use the built-in <code>[audio]</code> shortcode here.', 'pursuit' ),
			'std'     => '',
			'type'    => 'textarea'
		)
	);
...
function themo_ot_meta_box_post_format_link($array,$pages) {
	...
	$array['fields'] = array(
		...
		array(
			'id'      => '_format_link_url',
			'label'   => '',
			'desc'    => esc_html__( 'Link URL (ex. http://google.com)', 'pursuit' ),
			'std'     => '',
			'type'    => 'text'
		),
		array(
			...
			'label'   => '',
			'desc'    => esc_html__( 'Link Title (ex. Check out Google)', 'pursuit' ),
			'std'     => '',
			'type'    => 'text'
		),
		...
		array(
			...
			'label'       => esc_html__( 'Link Target', 'pursuit' ),
			'type'        => 'checkbox',
			'choices'     => array(
				array(
					'value'       => '_blank',
...
function themo_ot_meta_box_post_format_video($array,$pages) {
	...
	$array['fields'] = array(
		array(
			...
			'desc'    => sprintf( wp_kses_post( __( '(ex. http://vimeo.com/link-to-video). You can find a list of supported oEmbed sites in the %1$s.', 'pursuit' )), '<a href="http://codex.wordpress.org/Embeds" target="_blank">' . esc_html__( 'Wordpress Codex', 'pursuit' ) .'</a>' ),
			'std'     => '',
			'type'    => 'text'
		),
		array(
			...
			'desc'    => wp_kses_post(__( 'Use the built-in <code>[video]</code> shortcode here.', 'pursuit' )),
			'std'     => '',
			'type'    => 'textarea'
		)
	);
...
function th_register_required_plugins() {
	 ...
	 */
	$config = array(
		'id'           => 'pursuit',                 // Unique ID for hashing notices for multiple instances of TGMPA.
