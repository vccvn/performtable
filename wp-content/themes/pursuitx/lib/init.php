...
function themo_setup() {
    ...
    // Get the locale
    $locale = apply_filters('theme_locale', get_locale(), 'pursuit');
    // Try and load the user generated .mo outside of the theme directory.
    // It's name convention is pursuit-en_US.mo
    load_textdomain('pursuit', WP_LANG_DIR.'/pursuit/'.'pursuit'.'-'.$locale.'.mo');
    // Last, load our default (if we have one). Name convetion is just en_US.mo (not including theme name).
    load_theme_textdomain('pursuit', get_template_directory() . '/languages');


	// Register wp_nav_menu() menus (http://codex.wordpress.org/Function_Reference/register_nav_menus)
	register_nav_menus(array(
	'primary_navigation' => esc_html__('Primary Navigation', 'pursuit'),
	));
